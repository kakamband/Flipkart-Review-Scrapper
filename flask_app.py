...
def index():
    if request.method == 'POST':
        searchString = request.form['content'].replace(" ","") # obtaining the search string entered in the form(from html page)
        #content is the key => id() It will return Iphone 7
        try:
            dbConn = pymongo.MongoClient("mongodb://localhost:27017/")  # opening a connection to Mongo (local host mongoDb)
            db = dbConn['crawlerDB'] # connecting to the database called crawlerDB. If this Db is not there it will create for you.
            reviews = db[searchString].find({}) # searching the collection with the name same as the keyword in DB first
            if reviews.count() > 0: # if there is a collection with searched keyword and it has records in it
                ...
                flipkartPage = uClient.read() # reading the webpage No of result 200 in debugging
                uClient.close() # closing the connection to the web server. Because in production other people will not be able to use it. until you close your DB.
                flipkart_html = bs(flipkartPage, "html.parser") # parsing(structure) the webpage as HTML. Initially flipkart page is in byte format
